...
// Sistema de notificaciones con polling REST (45 segundos)
const notifications = ref([]);
let pollingInterval = null;

// Mapeo de tipos de notificación a colores y configuración visual
const tipoConfigMap = {
    'ticket': { icon: 'pi pi-ticket', color: 'text-blue-500', type: 'info' },
    'infraestructura': { icon: 'pi pi-exclamation-triangle', color: 'text-red-500', type: 'danger' },
    'wiki': { icon: 'pi pi-book', color: 'text-green-500', type: 'success' },
    'sistema': { icon: 'pi pi-info-circle', color: 'text-purple-500', type: 'system' },
    'DIRECTOR': { icon: 'pi pi-star-fill', color: 'text-yellow-500', type: 'reminder' }
};

const nivelConfigMap = {
    'urgente': { icon: 'pi pi-times-circle', color: 'text-red-600', type: 'danger' },
    'alerta': { icon: 'pi pi-exclamation-triangle', color: 'text-orange-500', type: 'warning' },
    'informativa': { icon: 'pi pi-info-circle', color: 'text-blue-500', type: 'info' }
};
...
const cargarNotificaciones = async () => {
    ...
    if (response.success) {
        notifications.value = response.data.map(notif => {
            const tipoConfig = tipoConfigMap[notif.tipo] || tipoConfigMap['sistema'];
            const nivelConfig = nivelConfigMap[notif.nivel] || nivelConfigMap['informativa'];

            // Priorizar configuración visual según nivel de urgencia
            const config = notif.nivel === 'urgente' ? nivelConfig : tipoConfig;

            return {
                id: notif.id,
                title: notif.titulo,
                content: notif.contenido.length > 80 ? notif.contenido.substring(0, 80) + '...' : notif.contenido,
                type: config.type,
                icon: config.icon,
                color: config.color,
                read: notif.leido,
...
const getBackgroundClass = (type) => {
    switch (type) {
        ...
        case 'task':
            return 'bg-cyan-50 dark:bg-cyan-900/20 hover:bg-cyan-100 dark:hover:bg-cyan-900/30';
        case 'security':
            return 'bg-gray-50 dark:bg-gray-900/20 hover:bg-gray-100 dark:hover:bg-gray-900/30';
        case 'financial':
            return 'bg-teal-50 dark:bg-teal-900/20 hover:bg-teal-100 dark:hover:bg-teal-900/30';
        case 'hr':
            return 'bg-pink-50 dark:bg-pink-900/20 hover:bg-pink-100 dark:hover:bg-pink-900/30';
        case 'inventory':
            return 'bg-indigo-50 dark:bg-indigo-900/20 hover:bg-indigo-100 dark:hover:bg-indigo-900/30';
        case 'reminder':
...
const onNotificationClick = async (item) => {
    ...
    const result = await marcarNotificacionLeida(item.id);
    ...
    // Cerrar popover y navegar
    opNotifications.value.hide();
    router.push(`/notificaciones/${item.id}`);
};
</script>
...
<template>
    <div class="layout-topbar">
        ...
        <div class="layout-topbar-actions">
            ...
            <button class="layout-topbar-action" @click="toggleSearch" title="Buscar (Ctrl+K)">
    ...
    <Menu ref="menuProfile" :model="profileItems" :popup="true" />
        ...
        <div class="flex flex-col gap-4">
            ...
            <div class="flex flex-col gap-2">
                <div v-for="(item, index) in notifications" :key="index"
                    ...
                    <i :class="[item.icon, item.color, 'text-xl mt-1']"></i>
                    <div class="flex flex-col flex-1">
                        <div class="flex justify-between items-center">
                            <span class="font-medium text-sm">{{ item.title }}</span>
                            <span v-if="!item.read" class="w-2 h-2 bg-blue-500 rounded-full" title="No leído"></span>
                        </div>
                        <span class="text-xs text-muted-color">{{ item.content }}</span>
                    </div>
                </div>
            </div>
        </div>
