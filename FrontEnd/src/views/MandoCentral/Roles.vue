...
const saveRol = () => {
    // La logica de guardado la maneja internamente RolFormDialog.
...
<template>
    <div class="flex flex-col gap-6">
        ...
        <div class="card">
            ...
            <DataTable :value="guiaNiveles" size="small" showGridlines stripedRows class="text-sm mb-4">
                <Column field="nivel" header="Nivel" class="w-32">
                    <template #body="slotProps">
                        <Tag :value="slotProps.data.nivel" :severity="slotProps.data.severity"
                ...
                <Column field="rol" header="Rol Oficial" class="w-48">
                    <template #body="slotProps">
                        <div class="flex items-center gap-2">
                            ...
                            <span class="font-semibold text-surface-700 dark:text-surface-100">{{ slotProps.data.rol
                                }}</span>
                        </div>
                    </template>
                </Column>
            ...
            <div
                class="flex flex-col gap-2 text-sm bg-surface-50 dark:bg-surface-900 p-4 rounded border border-surface-200 dark:border-surface-700">
                ...
                <ul class="list-disc list-inside text-surface-600 dark:text-surface-400 ml-1">
                    <li><strong>Nivel 200 (Director):</strong> Único con acceso a logs de auditoría maestra.
                        Indestructible.</li>
                    <li><strong>Validación:</strong> El sistema usa <code>minLevel</code>. Si un módulo requiere 186,
                        niveles
                        inferiores no tienen acceso.</li>
                </ul>
            </div>
        </div>
        ...
        <div class="card">
            ...
            <DataTableToolbar ref="toolbarRef" :dt="dt" :datos="roles" titulo-reporte="Roles del Sistema"
                nombre-archivo="roles" :columnas="columnas" :mostrar-exportacion="true" :mostrar-imprimir="true"
                :mostrar-refrescar="true" :mostrar-selector-columnas="true" :mostrar-buscador="true"
                @refrescar="cargarRoles" v-model:columnas-seleccionadas="columnas">
                <template #acciones-extra>
                    <Button icon="pi pi-plus" label="Nuevo Rol" rounded severity="primary" @click="openNew" />
                </template>
            </DataTableToolbar>

            <DataTable ref="dt" :value="roles" :loading="loading" v-model:filters="filtros"
                :globalFilterFields="['nombre', 'descripcion', 'nivel']" paginator :rows="10"
                :rowsPerPageOptions="[5, 10, 20, 50]" dataKey="id" filterDisplay="menu" showGridlines stripedRows
                ...
                <Column v-if="esColumnaVisible('nombre')" field="nombre" header="Nombre" sortable
                    ...
                    <template #body="{ data }">
                        <Tag :value="data.nombre" :severity="getNivelSeverity(data.nivel)"
                ...
                <Column v-if="esColumnaVisible('nivel')" field="nivel" header="Nivel" sortable style="min-width: 8rem">
                    <template #body="{ data }">
                    ...
                    <template #body="{ data }">
                        <div class="flex gap-2">
                            <Button icon="pi pi-pencil" size="small" severity="info" rounded outlined
                                @click="editarRol(data)" :disabled="data.nivel === 200" v-tooltip.top="'Editar'" />
                            <Button :icon="data.esta_activo ? 'pi pi-ban' : 'pi pi-check-circle'" size="small"
                                ...
                                @click="toggleActivarRol(data)" :disabled="data.nivel === 200"
            ...
            <RolFormDialog v-model:visible="rolDialog" :rolProp="rol" @saved="cargarRoles" />
        </div>
    </div>
</template>
