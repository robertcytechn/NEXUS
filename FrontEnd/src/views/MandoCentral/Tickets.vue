...
<template>
    <div class="flex flex-col gap-6">
        <div class="card">
            ...
            <Dialog v-model:visible="ticketDialog" :style="{ width: '700px' }" header="Detalles del Ticket"
                :modal="true">
                <div class="flex flex-col gap-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <InputText id="folio" :value="ticket.folio || 'Se generará automáticamente'" disabled fluid
                        ...
                        <div>
                            ...
                            <Select id="estado_ciclo" v-model="ticket.estado_ciclo" :options="estadosCiclo"
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <Select id="maquina" v-model="ticket.maquina" :options="maquinas" optionLabel="uid_sala"
                                optionValue="id" placeholder="Seleccione Máquina" filter fluid
                                :invalid="submitted && !ticket.maquina">
                                <template #option="slotProps">
                                    <div class="flex flex-col">
                                        <span class="font-bold">{{ slotProps.option.uid_sala }}</span>
                                        <span class="text-xs text-surface-500">{{ slotProps.option.casino_nombre }} - {{
                                            slotProps.option.modelo_nombre }}</span>
                                    </div>
                                </template>
                            </Select>
                            <small class="text-red-500" v-if="submitted && !ticket.maquina">La máquina es
                                obligatoria.</small>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div>
                            ...
                            <Select id="categoria" v-model="ticket.categoria" :options="categorias" optionLabel="label"
                                optionValue="value" placeholder="Seleccione" fluid
                        ...
                        <div>
                            ...
                            <InputText id="subcategoria" v-model="ticket.subcategoria"
                                placeholder="Ej. Billetero, Pantalla..." fluid />
                        </div>
                        <div>
                            <label for="prioridad" class="block font-bold mb-3">Prioridad</label>
                            <Select id="prioridad" v-model="ticket.prioridad" :options="prioridades" optionLabel="label"
                    ...
                    <div>
                        ...
                        <Textarea id="descripcion" v-model="ticket.descripcion_problema" rows="4" fluid
                            :invalid="submitted && !ticket.descripcion_problema" />
                        <small class="text-red-500" v-if="submitted && !ticket.descripcion_problema">La descripción es
                            obligatoria.</small>
                    </div>

                    <!-- Fila 5: Seguimiento (Solo visible si ya existe el ticket) -->
                    <div v-if="ticket.id"
                        class="border-t border-surface-200 dark:border-surface-700 pt-4 flex flex-col gap-4">
                        ...
                        <div>
                            ...
                            <Textarea id="notas" v-model="ticket.notas_seguimiento" rows="3" fluid
                                placeholder="Bitácora de acciones realizadas..." />
                        </div>

                        <div v-if="ticket.estado_ciclo === 'cerrado'">
                            <label for="cierre" class="block font-bold mb-3">Explicación de Cierre</label>
                            <Textarea id="cierre" v-model="ticket.explicacion_cierre" rows="3" fluid
                                class="bg-green-50 dark:bg-green-900/20" placeholder="Solución final aplicada..." />
                        </div>

                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                            <div>
                                <span class="font-bold text-surface-600">Reaperturas:</span> {{
                                ticket.contador_reaperturas }}
                            </div>
                            <div>
                                <span class="font-bold text-surface-600">Creado por:</span> {{ ticket.creado_por ||
                                'Sistema' }}
                            </div>
                        </div>
                    </div>
            ...
            <Dialog v-model:visible="historialDialog" :style="{ width: '900px' }" header="Historial de Bitácora Técnica"
                        ...
                        <template #content>
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <div>
                                    ...
                                    <Tag :value="ticketSeleccionado.estado_ciclo.toUpperCase()"
                                        :severity="getEstadoSeverity(ticketSeleccionado.estado_ciclo)" />
                                </div>
                                <div>
                                ...
                                <div>
                                    ...
                                    <Tag :value="ticketSeleccionado.prioridad.toUpperCase()"
                                        :severity="getPrioridadSeverity(ticketSeleccionado.prioridad)" />
                                </div>
                                <div>
                                ...
                                <div class="col-span-2">
                                    <p class="text-sm text-surface-600 dark:text-surface-400">Descripción del Problema
                                    </p>
