...
const saveTicket = async () => {
    console.log('🚀 INICIO saveTicket - ticket.value.id:', ticket.value.id);
    ...
    if (ticket.value.maquina && ticket.value.categoria && ticket.value.descripcion_problema?.trim()) {
        ...
        try {
            ...
            console.log('📝 Payload del ticket:', payload);
            console.log('🔍 ¿Es actualización o creación? ticket.value.id =', ticket.value.id);

            if (ticket.value.id) {
                console.log('📤 Entrando a ACTUALIZACIÓN (UPDATE)');
                ...
                console.log('📥 Entrando a CREACIÓN (CREATE)');
                ...
                console.log(`� Verificando payload.maquina:`, {
                    maquinaId: maquinaId,
                    tipo: typeof maquinaId,
                    esValido: !!maquinaId
                });
                console.log(`�🔧 Iniciando incremento de contador_fallas para máquina ID: ${maquinaId}`);
                
                if (!maquinaId) {
                    console.error('❌ No se encontró ID de máquina en el payload');
                    throw new Error('ID de máquina no válido');
                }
                
                try {
                    const fallasResponse = await api.post(`maquinas/${maquinaId}/incrementar-fallas/`);
                    console.log('✅ Respuesta del incremento de fallas:', fallasResponse.data);
                    ...
                    console.error('❌ Error completo al incrementar contador_fallas:', errorFallas);
                    console.error('📋 Detalles del error:', {
                        message: errorFallas.message,
                        response: errorFallas.response?.data,
                        status: errorFallas.response?.status,
                        url: `maquinas/${maquinaId}/incrementar-fallas/`
                    });
                    toast.add({ 
                        severity: 'warn', 
                        summary: 'Advertencia', 
    ...
    <div class="flex flex-col gap-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div class="surface-card border border-surface-200 dark:border-surface-700 rounded-lg p-6">
                    <div class="flex justify-between items-start mb-4">
                        <div>
                            <span class="block text-surface-500 dark:text-surface-400 font-medium mb-3">Tickets Abiertos</span>
                ...
                <div class="surface-card border border-surface-200 dark:border-surface-700 rounded-lg p-6">
                    <div class="flex justify-between items-start mb-4">
                        <div>
                            ...
                            <div class="text-surface-900 dark:text-surface-0 font-medium text-4xl">{{ estadisticas.criticos }}</div>
                        </div>
                ...
                <div class="surface-card border border-surface-200 dark:border-surface-700 rounded-lg p-6">
                    <div class="flex justify-between items-start mb-4">
                        <div>
                            <span class="block text-surface-500 dark:text-surface-400 font-medium mb-3">Sin Técnico</span>
                            <div class="text-surface-900 dark:text-surface-0 font-medium text-4xl">{{ estadisticas.sin_tecnico }}</div>
                        </div>
                        <div class="flex items-center justify-center bg-orange-100 dark:bg-orange-400/10 rounded-lg" style="width:3.5rem;height:3.5rem">
        ...
        <div class="card">
            ...
            <DataTableToolbar
                ...
                :dt="dt"
                :datos="tickets"
                titulo-reporte="Tickets Abiertos"
                nombre-archivo="tickets_abiertos"
                :columnas="columnas"
            ...
            >
                <template #acciones-extra>
                    <Button v-if="permisos.puedeAgregar" icon="pi pi-plus" label="Nuevo Ticket" rounded severity="primary" @click="openNew" />
                </template>
            </DataTableToolbar>

            <DataTable
                ref="dt" :value="tickets" :loading="loading" v-model:filters="filtros"
            ...
            >
                <template #empty><div class="text-center p-4">No hay tickets abiertos en este momento.</div></template>
                ...
                <Column v-if="esColumnaVisible('dias_abierto')" field="dias_abierto" header="Antigüedad" sortable style="min-width: 10rem">
                    <template #body="{ data }">
                        <div class="flex items-center gap-2 px-3 py-2 rounded-lg border-2" :class="getDiasClass(data.dias_abierto)">
                ...
                <Column v-if="esColumnaVisible('total_intervenciones')" field="total_intervenciones" header="Intervenciones" sortable style="min-width: 10rem">
                    <template #body="{ data }">
                        <div class="flex items-center gap-2">
                            <Tag :value="String(data.total_intervenciones)" :severity="data.total_intervenciones > 0 ? 'info' : 'secondary'" rounded>
                ...
                <Column v-if="esColumnaVisible('tecnico_asignado_nombre')" field="tecnico_asignado_nombre" header="Técnico" sortable style="min-width: 12rem">
                    <template #body="{ data }">
                ...
                <Column v-if="permisos.puedeEditar || permisos.puedeDesactivar" header="Acciones" :exportable="false" style="min-width: 10rem">
                    <template #body="{ data }">
                        <div class="flex gap-2">
                            <Button v-if="permisos.puedeEditar" icon="pi pi-pencil" size="small" severity="info" rounded outlined @click="editarTicket(data)" v-tooltip.top="'Editar'" />
                            <Button v-if="permisos.puedeDesactivar" icon="pi pi-ban" size="small" severity="warning" rounded outlined @click="desactivarTicket(data)" v-tooltip.top="'Desactivar'" />
                        </div>
                    </template>
                </Column>
            </DataTable>
            ...
            <!-- Dialog Crear/Editar -->
            <Dialog v-model:visible="ticketDialog" :style="{ width: '700px' }" :breakpoints="{ '768px': '95vw' }" header="Datos del Ticket" :modal="true">
                <div class="flex flex-col gap-6">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            <label for="folio" class="block font-bold mb-3">Folio</label>
                            <InputText id="folio" name="folio" :value="ticket.folio || 'Se generará automáticamente'" disabled fluid class="bg-surface-100 dark:bg-surface-800 font-mono" />
                        </div>
                        <div>
                            <label for="estado_ciclo" class="block font-bold mb-3">Estado</label>
                            <Select id="estado_ciclo" name="estado_ciclo" v-model="ticket.estado_ciclo" :options="estadosCiclo" optionLabel="label" optionValue="value" fluid />
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <Select 
                                ...
                                v-model="ticket.tecnico_asignado" 
                                :options="tecnicos" 
                                optionLabel="nombres" 
                                optionValue="id" 
                            ...
                            >
                                <template #option="slotProps">
                                    <div class="flex flex-col">
                                        <span class="font-bold">{{ slotProps.option.nombres }} {{ slotProps.option.apellido_paterno }}</span>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div>
                            ...
                            <Select id="categoria" name="categoria" v-model="ticket.categoria" :options="categorias" optionLabel="label" optionValue="value" fluid :invalid="submitted && !ticket.categoria" />
                        ...
                        <div>
                            ...
                            <InputText id="subcategoria" name="subcategoria" v-model="ticket.subcategoria" fluid placeholder="Ej: Billetero trabado" />
                        </div>
                        <div>
                            <label for="prioridad" class="block font-bold mb-3">Prioridad</label>
                            <Select id="prioridad" name="prioridad" v-model="ticket.prioridad" :options="prioridades" optionLabel="label" optionValue="value" fluid />
                        </div>
                    </div>

                    <div>
                        <label for="descripcion_problema" class="block font-bold mb-3">Descripción del Problema</label>
                        <Textarea id="descripcion_problema" name="descripcion_problema" v-model="ticket.descripcion_problema" rows="4" fluid :invalid="submitted && !ticket.descripcion_problema" />
                    ...
                    <div>
                        ...
                        <Textarea id="notas_seguimiento" name="notas_seguimiento" v-model="ticket.notas_seguimiento" rows="2" fluid />
                    </div>
                </div>
                <template #footer>
            ...
            <!-- Modal de Detalle con Bitácoras -->
            <Dialog v-model:visible="detalleDialog" :style="{ width: '1000px' }" :breakpoints="{ '960px': '95vw' }" header="Detalle del Ticket" :modal="true" :maximizable="true">
                <div v-if="ticketDetalle" class="flex flex-col gap-5">
                    ...
                    <div class="surface-card border-2 border-primary-200 dark:border-primary-900 rounded-xl p-5 bg-gradient-to-br from-primary-50 to-white dark:from-primary-950 dark:to-surface-900">
                        <div class="flex flex-col md:flex-row items-start md:items-center gap-3 md:gap-4 mb-5">
                            <div class="flex items-center gap-3 w-full md:w-auto">
                                <div class="flex items-center justify-center bg-primary-500 rounded-xl shadow-lg shrink-0" style="width:3rem;height:3rem">
                                    <i class="pi pi-ticket text-white text-xl"></i>
                                </div>
                                <div class="flex-1 min-w-0">
                                    <h3 class="text-xl md:text-2xl font-bold text-surface-900 dark:text-surface-0 mb-1 truncate">{{ ticketDetalle.folio }}</h3>
                                    <p class="text-surface-600 dark:text-surface-400 font-medium text-sm md:text-base truncate">
                            ...
                            <div class="flex gap-2 shrink-0">
                                <Tag :value="ticketDetalle.prioridad.toUpperCase()" :severity="getPrioridadSeverity(ticketDetalle.prioridad)" class="text-sm md:text-base px-3 md:px-4 py-1.5 md:py-2 font-bold" rounded />
                                <Tag :value="ticketDetalle.estado_ciclo.toUpperCase()" :severity="getEstadoSeverity(ticketDetalle.estado_ciclo)" class="text-sm md:text-base px-3 md:px-4 py-1.5 md:py-2 font-bold" rounded />
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
                            <div class="rounded-lg p-3 border-2" :class="getDiasClass(ticketDetalle.dias_abierto)">
                                ...
                                <span class="text-xs font-semibold">{{ getDiasLabel(ticketDetalle.dias_abierto) }}</span>
                            </div>
                            ...
                            <div class="bg-white dark:bg-surface-800 rounded-lg p-3 border border-surface-200 dark:border-surface-700">
                                <div class="flex items-center gap-2 mb-2">
                                    ...
                                    <span class="text-surface-500 dark:text-surface-400 text-xs font-semibold">Intervenciones</span>
                                </div>
                                <span class="font-bold text-indigo-700 dark:text-indigo-400 text-2xl">{{ ticketDetalle.total_intervenciones }}</span>
                            </div>

                            <div class="bg-white dark:bg-surface-800 rounded-lg p-3 border border-surface-200 dark:border-surface-700">
                                <div class="flex items-center gap-2 mb-2">
                                    <i class="pi pi-user text-teal-500 text-sm"></i>
                                    <span class="text-surface-500 dark:text-surface-400 text-xs font-semibold">Técnico</span>
                                </div>
                                <span class="font-bold text-surface-900 dark:text-surface-0 text-sm">{{ ticketDetalle.tecnico_asignado_nombre || 'Sin asignar' }}</span>
                            </div>
                            ...
                            <div class="bg-white dark:bg-surface-800 rounded-lg p-3 border border-surface-200 dark:border-surface-700">
                                ...
                                <span class="text-xs text-surface-700 dark:text-surface-300">{{ formatearFecha(ticketDetalle.creado_en) }}</span>
                            </div>

                            <div class="col-span-1 sm:col-span-2 md:col-span-4 bg-white dark:bg-surface-800 rounded-lg p-3 border border-surface-200 dark:border-surface-700">
                                <div class="flex items-center gap-2 mb-2">
                                    <i class="pi pi-align-left text-surface-500 text-sm"></i>
                                    <span class="text-surface-500 dark:text-surface-400 text-xs font-semibold">Descripción del Problema</span>
                                </div>
                                <p class="text-sm text-surface-700 dark:text-surface-300">{{ ticketDetalle.descripcion_problema }}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="surface-card border border-surface-200 dark:border-surface-700 rounded-lg p-6">
                        <div class="flex items-center gap-2 mb-4">
                            ...
                            <h4 class="font-bold text-xl text-surface-900 dark:text-surface-0">Historial de Intervenciones ({{ bitacoras.length }})</h4>
                        </div>
                        ...
                        <div v-else class="space-y-4">
                            <div v-for="(bitacora, idx) in bitacoras" :key="idx" class="bg-blue-50 dark:bg-blue-950 rounded-lg p-3 md:p-4 border-l-4 border-blue-500">
                                <div class="flex flex-col sm:flex-row justify-between items-start gap-1 mb-2">
                                    <div class="flex items-center gap-2">
                                        ...
                                        <span class="font-bold text-surface-900 dark:text-surface-0">{{ bitacora.tecnico_nombre }}</span>
                                    </div>
                                    <span class="text-xs text-surface-500">{{ formatearFecha(bitacora.fecha_registro) }}</span>
                                </div>
                                <p class="text-xs md:text-sm text-surface-700 dark:text-surface-300 mb-3">{{ bitacora.descripcion_trabajo }}</p>
                                <div class="flex flex-wrap gap-2">
                                    <Tag :value="bitacora.tipo_intervencion" severity="secondary" class="text-xs" icon="pi pi-cog" />
                    ...
                    <div v-if="!ticketEstaCerrado && permisos.puedeAgregarBitacora" class="surface-card border-2 border-primary-200 dark:border-primary-800 rounded-lg p-6">
                        ...
                        <div class="grid grid-cols-1 gap-5">
                            ...
                            <div>
                                ...
                                <Textarea 
                                    id="descripcion"
                                    name="descripcion"
                                    v-model="nuevaBitacora.descripcion_trabajo"
                                    rows="4"
                                    placeholder="Detalle las acciones realizadas, pruebas ejecutadas y componentes revisados..."
                                    :invalid="submittedBitacora && !nuevaBitacora.descripcion_trabajo"
                                    class="w-full"
                                />
                            ...
                            <div>
                                ...
                                <small v-if="submittedBitacora && !nuevaBitacora.estado_maquina_resultante" class="text-red-500">
                            ...
                            <div class="border-2 border-dashed border-surface-300 dark:border-surface-600 rounded-lg p-4">
                                <div class="flex items-center gap-3 mb-3">
                                    <Checkbox 
                                        v-model="nuevaBitacora.finaliza_ticket" 
                                        :binary="true" 
                                        inputId="finaliza"
                                    />
                                ...
                                <div v-if="nuevaBitacora.finaliza_ticket">
                                    ...
                                    <div class="mt-4">
                                        ...
                                        <Textarea 
                                            ...
                                            name="explicacion_cierre"
                                            v-model="nuevaBitacora.explicacion_cierre"
                                            rows="3"
                                            ...
                                            class="w-full"
                                        />
                                        <small v-if="submittedBitacora && nuevaBitacora.finaliza_ticket && !nuevaBitacora.explicacion_cierre" class="text-red-500">
                            ...
                            <div class="flex flex-col-reverse sm:flex-row gap-3 justify-end">
                                <Button 
                                    label="Cancelar" 
                                    icon="pi pi-times" 
                                    @click="cancelarFormularioBitacora"
                                    severity="secondary"
                                    outlined
                                />
                                <Button 
                                    label="Guardar Intervención" 
                                    icon="pi pi-save" 
                                    @click="guardarBitacora"
                    ...
                    <div v-else-if="ticketEstaCerrado" class="surface-card border border-surface-200 dark:border-surface-700 rounded-lg p-6">
